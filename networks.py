    ...
    def __init__(
        ...
        **kwargs,
        ...
        y_seg_src = layers.SpatialTransformer(
            interp_method="linear", name=f"{name}_seg_transformer"
        )([seg_src, seg_flow])
        ...
        if bidir_labels:
            ...
            y_seg_trg = layers.SpatialTransformer(
                interp_method="linear", name=f"{name}_seg_neg_transformer"
            )(
...
class InstanceDense(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        # warp image with flow field
        y_source = layers.SpatialTransformer(
            interp_method="linear", name="transformer"
        )([source, pos_flow])
...
class ProbAtlasSegmentation(ne.modelio.LoadableModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if gaussian_likelihood:
            ...
            uloglhood = KL.Lambda(
                lambda x: unnorm_loglike(*x), name="unsup_likelihood"
    ...
    def get_likelihood_warp_model(self):
        ...
        if self.references.gaussian_likelihood:
            ...
            outputs = [self.references.uloglhood, self.outputs[-1]]
            return tf.keras.Model(self.inputs, outputs)
...
def _conv_block(
    ...
    if batch_norm:
        convolved = KL.BatchNormalization(name=f"{name}_batch_norm")(convolved)
...
def _upsample_block(x, connection, factor=2, name=None, batch_norm = False):
    """
    ...
    if batch_norm:
        upsampled = KL.BatchNormalization(name=f"{name}_batch_norm")(upsampled)
